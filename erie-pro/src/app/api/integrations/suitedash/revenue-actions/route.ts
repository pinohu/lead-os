...
import {
  ...
  suiteDashOperationStatuses,
} from "@/lib/suitedash-operational-sync"

export const dynamic = "force-dynamic"

const StatusSchema = z.enum(suiteDashOperationStatuses)
...
const StatusCallbackSchema = z.object({
  ...
  externalSystem: z.string().max(120).default("suitedash"),
  completedOperations: z.array(z.string().min(1)).optional(),
})
...
function hasSuiteDashAccess(request: NextRequest) {
  const token = process.env.SUITEDASH_REVENUE_ACTION_TOKEN || process.env.REVENUE_ACTIONS_API_TOKEN
...
function serializeForSuiteDash(item: {
  id: string
  eventType: string
  ...
  return {
    ...buildSuiteDashOperationalPackage({
      action: automationPayload,
      status: metadata.status,
    }),
    outcome,
    createdAt: item.createdAt.toISOString(),
    completedOperations: metadata.completedOperations ?? [],
  }
}

export async function GET(request: NextRequest) {
  if (!hasSuiteDashAccess(request)) {
  ...
  try {
    ...
    const packages = records
      .map(serializeForSuiteDash)
      .filter((item) => (status === "all" ? true : item.status === status))
      .filter((item) => shouldRouteToSuiteDash(item.action))
      .slice(0, limit)
    ...
    if (isDatabaseUnavailable(error)) {
      return NextResponse.json({
        ...
        count: 0,
        packages: [],
        warning: "DATABASE_URL is not configured; SuiteDash revenue actions cannot be loaded in this environment.",
      })
    }
    logger.error("api/integrations/suitedash/revenue-actions", "Failed to load SuiteDash revenue actions", error)
    return NextResponse.json({ success: false, error: "Could not load SuiteDash revenue actions" }, { status: 500 })
  }
}
    ...
    const updated = await prisma.offerInteraction.update({
      ...
      data: {
        metadata: {
          ...
          completedOperations: parsed.data.completedOperations ?? metadata.completedOperations ?? [],
          statusUpdatedAt: new Date().toISOString(),
        } as Prisma.InputJsonValue,
      },
    })

    return NextResponse.json({ success: true, package: serializeForSuiteDash(updated) })
  } catch (error) {
    if (isDatabaseUnavailable(error)) {
      return NextResponse.json(
        { success: false, error: "DATABASE_URL is not configured; SuiteDash revenue action status cannot be updated in this environment." },
        { status: 503 },
      )
    }
    logger.error("api/integrations/suitedash/revenue-actions", "Failed to update SuiteDash revenue action", error)
    return NextResponse.json({ success: false, error: "Could not update SuiteDash revenue action" }, { status: 500 })
  }
}
