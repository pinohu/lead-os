...
import { createContact, listContacts } from "@/lib/integrations/flowlu-adapter";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.error("flowlu/contacts POST failed", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
...
export async function GET(request: Request) {
  ...
  try {
    ...
    logger.error("flowlu/contacts GET failed", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
      { data: null, error: { code: "LIST_FAILED", message: "Failed to list contacts" }, meta: null },
      { status: 500, headers },
    );
  }
}
