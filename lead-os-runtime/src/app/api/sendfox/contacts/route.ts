...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Without a specific lookup, return empty — contacts are retrieved by email
    return NextResponse.json(
      { data: [], error: null, meta: { tenantId: tenantId ?? session.email } },
      { headers },
    );
  } catch (err) {
    logger.error("sendfox/contacts GET failed", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.error("sendfox/contacts POST failed", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
