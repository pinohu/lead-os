...
import { getRevenueReport } from "@/lib/monetization-engine";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const report = getRevenueReport(tenantId, period);
    ...
    return NextResponse.json(
      { data: null, error: { code: "INTERNAL_ERROR", message: "Failed to generate revenue report" }, meta: null },
      { status: 500, headers },
    );
  }
}
