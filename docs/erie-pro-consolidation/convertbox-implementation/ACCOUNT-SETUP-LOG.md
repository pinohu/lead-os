...
Fifteenth correction recorded after full ConvertBox implementation: `scripts/deploy-erie-convertbox-112-drafts.js` created or updated service-aware inactive ConvertBox drafts for all 112 live Erie.Pro services. The script clones the appropriate family template, applies service-specific copy, branches, fields, targeting, profile/teaser assets, metadata, and inactive draft status, and writes `ERIE-CONVERTBOX-112-DRAFT-DEPLOYMENT.json` plus `.md`. The independent account audit in `scripts/audit-erie-convertbox-112-drafts.js` verified 112 expected services, 112 service drafts found, 112 unique service slugs, 112 passed drafts, 0 failed drafts, and no duplicates. No generated service draft is active; all still require visual/mobile preview and test submissions before activation.
...
Seventeenth correction recorded after account activation: `scripts/sync-erie-convertbox-website-targets.js` updated all 112 service-aware ConvertBox boxes so their URL targeting matches the current Erie.Pro public route slugs. The same sync fetched the Erie.Pro ConvertBox public install UUID from the authenticated site API and wrote `ERIE-CONVERTBOX-WEBSITE-TARGET-SYNC.json` plus `.md`.

`scripts/activate-erie-convertbox-service-boxes.js` then activated only the 112 full service-aware boxes, leaving older prototype boxes out of scope. Activation verification wrote `ERIE-CONVERTBOX-112-ACTIVATION.json` plus `.md` and reported 112 attempted, 112 activated, 0 failed. The Erie.Pro loader now includes the public ConvertBox UUID fallback so the website can load ConvertBox without manual Vercel environment-variable work.
