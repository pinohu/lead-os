...
Some Markdown and Kubernetes samples still use **`leadgen-os.com`** or **`lead-os.example`** (status URL, support portal, example ingress hosts). Those are illustrative defaults for a generic deployment - they are not a promise that those DNS names exist for your fork. Production values should always come from **`NEXT_PUBLIC_SITE_URL`**, **`NEXT_PUBLIC_SUPPORT_EMAIL`**, and your real status/help URLs. The in-app marketing pages prefer **`tenantConfig`** / `NEXT_PUBLIC_*` fallbacks over hardcoded production domains.
