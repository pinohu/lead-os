...
export async function GET(request: Request) {
  ...
  try {
    ...
    const stats = await getAcumbamailStats(tenantId);

    return NextResponse.json(
      { data: { totalCampaigns: stats.totalCampaigns }, error: null, meta: { tenantId: tenantId ?? session.email } },
      { headers },
    );
  } catch (err) {
    logger.error("acumbamail/campaigns GET failed", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.error("acumbamail/campaigns POST failed", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json(
