import { NextResponse } from "next/server";
import { z } from "zod";
import { buildCorsHeaders } from "@/lib/cors";
import { requireOperatorApiSession } from "@/lib/operator-auth";
import {
  requestCallback,
  listCallbackRequests,
} from "@/lib/integrations/novocall-adapter";

const RequestCallbackSchema = z.object({
  name: z.string().min(1),
  phone: z.string().min(1),
  email: z.string().email().optional(),
  company: z.string().optional(),
  preferredTime: z.string().datetime().optional(),
  source: z.string().optional(),
  page: z.string().optional(),
  tenantId: z.string().optional(),
});

export async function POST(request: Request) {
  const headers = buildCorsHeaders(request.headers.get("origin"));

  const { response: authError } = await requireOperatorApiSession(request);
  if (authError) return authError;

  try {
    const raw = await request.json();
    const validation = RequestCallbackSchema.safeParse(raw);

    if (!validation.success) {
      return NextResponse.json(
        {
          data: null,
          error: { code: "VALIDATION_ERROR", message: "Invalid input", details: validation.error.issues },
          meta: null,
        },
        { status: 422, headers },
      );
    }

    const callback = await requestCallback(validation.data);
    return NextResponse.json(
      { data: callback, error: null, meta: null },
      { status: 201, headers },
    );
  } catch (err) {
    console.error("[novocall/callbacks POST]", err instanceof Error ? err.message : err);
    return NextResponse.json(
      { data: null, error: { code: "REQUEST_FAILED", message: "Failed to request callback" }, meta: null },
      { status: 500, headers },
    );
  }
}

export async function GET(request: Request) {
  const headers = buildCorsHeaders(request.headers.get("origin"));

  const { response: authError } = await requireOperatorApiSession(request);
  if (authError) return authError;

  try {
    const url = new URL(request.url);
    const filter: Record<string, string | undefined> = {};

    for (const key of ["status", "source", "tenantId", "dateFrom", "dateTo"]) {
      const value = url.searchParams.get(key);
      if (value) filter[key] = value;
    }

    const hasFilter = Object.keys(filter).length > 0;
    const callbacks = await listCallbackRequests(hasFilter ? filter : undefined);

    return NextResponse.json(
      { data: callbacks, error: null, meta: { count: callbacks.length } },
      { headers },
    );
  } catch (err) {
    console.error("[novocall/callbacks GET]", err instanceof Error ? err.message : err);
    return NextResponse.json(
      { data: null, error: { code: "LIST_FAILED", message: "Failed to list callbacks" }, meta: null },
      { status: 500, headers },
    );
  }
}
